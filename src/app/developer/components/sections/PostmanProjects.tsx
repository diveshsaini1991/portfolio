...
export default function PostmanProjects() {
  ...
  const renderResponseAsHTML = (data: any) => {
    if (data.error) {
      return (
        <div className="p-6 bg-gradient-to-br from-red-50 to-red-100 dark:from-gray-800 dark:to-gray-900 rounded-lg border-l-4 border-red-500">
          <div className="text-center">
            <div className="text-6xl mb-4">⚠️</div>
            <h1 className="text-3xl font-bold text-red-700 dark:text-white mb-2">{data.error}</h1>
            <p className="text-red-600 dark:text-gray-300 text-lg mb-4">{data.message}</p>
            {data.details && (
              <div className="bg-white dark:bg-gray-950 rounded p-4 text-left border border-red-200 dark:border-gray-700">
                <h3 className="text-sm text-red-500 dark:text-gray-400 mb-2 font-semibold">Details:</h3>
      ...
      return (
        <div className="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-900">
          <div className="mb-6 text-center">
            <h1 className="text-3xl font-bold text-gray-800 dark:text-white mb-2">API Response</h1>
            <p className="text-gray-600 dark:text-gray-400">Found {data.count} result{data.count !== 1 ? 's' : ''}</p>
          </div>
          <div className="space-y-4">
            {data.results.map((item: any, idx: number) => (
              <div key={idx} className="bg-white dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500 transition-colors shadow-sm">
                <div className="flex items-start justify-between mb-2">
                  ...
                  {item.type && (
                    <span className={`inline-block px-2 py-1 rounded text-xs font-semibold ml-2 ${
                      item.type === 'fullstack' ? 'bg-purple-100 dark:bg-purple-600 text-purple-700 dark:text-white' :
                      item.type === 'backend' ? 'bg-green-100 dark:bg-green-600 text-green-700 dark:text-white' :
                      item.type === 'frontend' ? 'bg-blue-100 dark:bg-blue-600 text-blue-700 dark:text-white' :
                      item.type === 'internship' ? 'bg-green-100 dark:bg-green-600 text-green-700 dark:text-white' :
                      item.type === 'fulltime' ? 'bg-blue-100 dark:bg-blue-600 text-blue-700 dark:text-white' :
                ...
                {(item.role || item.duration) && (
                  <div className="flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400 mb-3">
                    {item.role && (
                      <span><strong className="text-gray-700 dark:text-gray-300">Role:</strong> {item.role}</span>
                    )}
                    {item.duration && (
                      <span><strong className="text-gray-700 dark:text-gray-300">Duration:</strong> {item.duration}</span>
                    )}
                  </div>
                )}
                ...
                {item.position && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">
                    <strong className="text-gray-700 dark:text-gray-300">Position:</strong> {item.position}
                  </p>
                )}
                {item.location && (
                  <p className="text-sm text-gray-500 dark:text-gray-400 mb-3">
                    <strong className="text-gray-700 dark:text-gray-300">Location:</strong> {item.location}
                  </p>
                )}
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.techStack.map((tech: string, i: number) => (
                        <span key={i} className="px-2 py-1 bg-blue-100 dark:bg-blue-600 text-blue-700 dark:text-white rounded text-xs">
                          {tech}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="mb-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.technologies.map((tech: string, i: number) => (
                        <span key={i} className="px-2 py-1 bg-purple-100 dark:bg-purple-600 text-purple-700 dark:text-white rounded text-xs">
                          {tech}
                        </span>
                      ))}
                    </div>
                ...
                {item.highlights && item.highlights.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Key Highlights:</p>
                    <ul className="space-y-1 text-xs text-gray-600 dark:text-gray-300">
                      {item.highlights.map((highlight: string, i: number) => (
                        <li key={i} className="flex items-start gap-2">
                          <span className="text-green-600 dark:text-green-400 mt-0.5">✓</span>
                          <span dangerouslySetInnerHTML={{ 
                            __html: highlight.replace(/\*\*(.*?)\*\*/g, '<strong class="text-gray-800 dark:text-white">$1</strong>') 
                          }} />
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-gray-200 dark:border-gray-600">
                    {item.github && (
                      <a
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-900 text-gray-700 dark:text-white rounded text-xs transition-colors"
                      >
    ...
    return (
      <div className="p-6 bg-gray-100 dark:bg-gray-800 rounded-lg">
        <pre className="text-gray-700 dark:text-gray-300 text-sm">{JSON.stringify(data, null, 2)}</pre>
      </div>
    );
  };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden shadow-sm dark:shadow-none">
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-600 dark:from-orange-600 dark:to-orange-500 px-3 sm:px-4 py-2 sm:py-3 flex items-center gap-2 sm:gap-3">
      ...
      {/* Tabs */}
      <div className="bg-slate-50 dark:bg-gray-800 px-2 sm:px-4 border-b border-gray-200 dark:border-gray-700 flex gap-0.5 sm:gap-1">
        <button
      ...
      <div className="bg-slate-50 dark:bg-gray-800 px-2 sm:px-4 py-2 sm:py-3 border-b border-gray-200 dark:border-gray-700 space-y-2">
        {/* Rate Limit */}
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2 sm:gap-4 text-xs sm:text-sm">
            <span className="text-gray-500 dark:text-gray-400 text-[10px] sm:text-sm">Rate Limit:</span>
        ...
        {/* Cache Stats */}
        <div className="flex items-center justify-between flex-wrap gap-2 pt-2 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2 sm:gap-4 text-xs sm:text-sm flex-wrap">
            <span className="text-gray-500 dark:text-gray-400 text-[10px] sm:text-sm">Cache Stats:</span>
            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
              <span>Hit Rate:</span>
              <span className={`font-mono font-bold ${cacheStats.hitRate > 70 ? 'text-green-600 dark:text-green-400' : cacheStats.hitRate > 40 ? 'text-yellow-600 dark:text-yellow-400' : 'text-gray-500 dark:text-gray-400'}`}>
            ...
            <div className="flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
              <span>Requests:</span>
              <span className="font-mono text-gray-700 dark:text-gray-300">{cacheStats.totalRequests}</span>
            </div>
            <span className="text-gray-300 dark:text-gray-600 hidden md:inline">|</span>
            <div className="hidden md:flex items-center gap-1.5 text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">
              <span>Saved:</span>
              <span className="font-mono text-green-600 dark:text-green-400">{(cacheStats.timeSaved / 1000).toFixed(1)}s</span>
            </div>
          </div>
          <button
            ...
            className={`px-2 sm:px-3 py-1 rounded text-[10px] sm:text-xs transition-all flex items-center gap-1 ${
              cacheCleared
                ? 'bg-green-100 dark:bg-green-600/30 border border-green-400 dark:border-green-500 text-green-700 dark:text-green-400'
                : 'bg-red-50 dark:bg-red-600/20 hover:bg-red-100 dark:hover:bg-red-600/30 border border-red-300 dark:border-red-600/50 text-red-600 dark:text-red-400'
            }`}
        ...
        {/* Alerts */}
        {rateLimitStatus.remaining === 0 && (
          <div className="px-2 sm:px-3 py-1.5 sm:py-2 bg-red-100 dark:bg-red-900/30 border border-red-300 dark:border-red-500 rounded text-[10px] sm:text-xs text-red-700 dark:text-red-400">
        ...
        {cacheCleared && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="px-2 sm:px-3 py-1.5 sm:py-2 bg-green-100 dark:bg-green-900/30 border border-green-300 dark:border-green-500 rounded text-[10px] sm:text-xs text-green-700 dark:text-green-400 flex items-center gap-2"
          >
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className=" dark:bg-gray-850 px-2 sm:px-4 py-2 sm:py-3 border-b border-gray-200 dark:border-gray-700">
          ...
          <div className="flex flex-col sm:flex-row gap-2 mb-2 sm:mb-3">
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 sm:px-6 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 dark:disabled:bg-gray-600 text-white rounded font-medium text-xs sm:text-sm transition-colors sm:hidden"
              >
                {requestState.isLoading ? '...' : 'Send'}
              </button>
            </div>
            <div className="flex-1 px-2 sm:px-4 py-1.5 sm:py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded font-mono text-[10px] sm:text-sm flex items-center overflow-x-auto">
              {buildEndpoint()}
            </div>
            <button
              onClick={executeRequest}
              disabled={requestState.isLoading}
              className="hidden sm:block px-6 py-2 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 dark:disabled:bg-gray-600 text-white rounded font-medium text-sm transition-colors"
            >
          ...
          <div className="flex flex-wrap gap-1.5 sm:gap-2 items-center">
            <span className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 self-center">Params:</span>
            {activeTab === 'projects' ? (
              <>
                <select
                  value={filters.projectType}
                  onChange={(e) => setFilters(prev => ({ ...prev, projectType: e.target.value }))}
                  className="px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded text-[10px] sm:text-xs border-none focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                ...
                <input
                  ...
                  className="px-2 sm:px-3 py-1 sm:py-1.5 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded text-[10px] sm:text-xs border-none focus:outline-none focus:ring-1 focus:ring-blue-500 placeholder-gray-400 dark:placeholder-gray-500 w-24 sm:w-auto"
                />
              </>
            ) : (
              <select
            ...
            <button
              ...
              className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded text-[10px] sm:text-xs font-medium transition-all flex items-center gap-1 ${
                cacheEnabled 
                  ? 'bg-green-100 dark:bg-green-900/30 border border-green-400 dark:border-green-600/50 text-green-700 dark:text-green-400 hover:bg-green-200 dark:hover:bg-green-900/50' 
                  : 'bg-gray-200 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-600 dark:text-gray-400 hover:bg-gray-300 dark:hover:bg-gray-600'
              }`}
        ...
        {/* Response Section */}
        <div className="flex-1 overflow-auto bg-white dark:bg-gray-900 p-2 sm:p-4">
            ...
            {!requestState.isLoading && requestState.response && (
              ...
              >
                ...
                <div className="mb-3 sm:mb-4 flex items-center gap-1.5 sm:gap-3 flex-wrap">
                  ...
                  <span className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400">Time:</span>
                  ...
                  {requestState.cacheStatus === 'HIT' && (
                    <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 rounded text-[10px] sm:text-xs flex items-center gap-1">
                      🟢 Cache HIT
                  ...
                  {requestState.cacheStatus === 'MISS' && cacheEnabled && (
                    <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-300 rounded text-[10px] sm:text-xs">
                      🔴 Cache MISS
                    </span>
                  )}
                  {requestState.response.status === 429 && (
                    <span className="px-1.5 sm:px-2 py-0.5 sm:py-1 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 rounded text-[10px] sm:text-xs">
                      ⚠️ Rate Limited
                    </span>
                  )}
                ...
                <div className="mb-3 sm:mb-4">
                  <div className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mb-2 flex items-center gap-2">
                    <span>Response Headers</span>
                    <span className="text-gray-400 dark:text-gray-600">({Object.keys(requestState.response.headers).length})</span>
                  </div>
                  <div className="bg-slate-50 dark:bg-gray-800 rounded p-2 sm:p-3 font-mono text-[10px] sm:text-xs space-y-1 overflow-x-auto border border-gray-200 dark:border-gray-700">
                    {Object.entries(requestState.response.headers).map(([key, value]) => (
                      <div key={key} className="flex gap-2">
                        <span className="text-blue-600 dark:text-blue-400 whitespace-nowrap">{key}:</span>
                ...
                {/* Response Body */}
                <div>
                  <div className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-400 mb-2 flex items-center justify-between flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-1 sm:gap-2">
                      ...
                      {viewMode === 'json' && (
                        <button
                          ...
                          className="px-2 sm:px-3 py-0.5 sm:py-1 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 rounded text-[10px] sm:text-xs transition-colors"
                        >
                  ...
                  {viewMode === 'json' ? (
                    <div className="bg-slate-50 dark:bg-gray-800 rounded p-2 sm:p-4 font-mono text-[10px] sm:text-xs overflow-x-auto border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded overflow-auto max-h-64 sm:max-h-96 border border-gray-200 dark:border-gray-700">
